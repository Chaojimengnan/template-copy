...
bool file_operation::copy_directory()
{
    ...
    if (!split_str(base_cmd, source_str, target_str, ':')) return false;
    ...
    auto make_target = [&](fs::path source_path) -> fs::path {
        ...
        {
            if (find_source) target_path /= i;
            if (i == source_str) find_source = true;
        }
        return target_path;
    };
    ...
    for (auto &&i : re_iter)
    {
        ...
        if (current_source_path.extension() != ".tem")
        {
            ...
            else fs::copy_file(current_source_path, current_target_path);
...
bool file_operation::set_variable()
{
    ...
    if (!split_str(base_cmd, var_name, var_val, '=')) return false;
    if (!tem_oper->set_variable(var_name, var_val)) return false;
    return true;
}
...
bool file_operation::split_str
    (const std::string& sc_str, std::string& lsub_str, std::string& rsub_str, char split_char)
{
    auto split_point = sc_str.find_first_of(split_char);
    if (split_point == std::string::npos) return false;
